  ...
  return <>
    ...
    <section id="story-timeline" className="section story-timeline-section">
      <div className="container">
        <div className="story-timeline-heading"><span className="eyebrow">{t("Our journey")}</span><h2>{t("Built step by step, with communities at the centre.")}</h2></div>
        <div className="story-timeline">
          {milestones.map((item, index) => <article className="story-milestone" key={item.year}>
            <div className="story-milestone-year">{t(item.year)}</div><div className="story-milestone-mark"><span /></div><div><span>{String(index + 1).padStart(2, "0")}</span><h3>{t(item.title)}</h3><p>{t(item.text)}</p></div>
          </article>)}
        </div>
      </div>
    </section>
