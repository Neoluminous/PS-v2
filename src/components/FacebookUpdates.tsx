...
export default function FacebookUpdates() {
  ...
  return <section className="facebook-updates" aria-labelledby="facebook-updates-title">
    <div className="facebook-updates-copy">
      <span className="facebook-live-label"><i />{t("Recent updates")}</span>
      <h2 id="facebook-updates-title">{t("Latest news & updates.")}</h2>
      <p>{t("Recent activities and announcements from Punjabi Samvad.")}</p>
      <p className="facebook-note">{t("This timeline is loaded live from Facebook. If Facebook blocks the preview because of your browser or privacy settings, open the page directly.")}</p>
      <a className="button" href={facebookPage} target="_blank" rel="noreferrer">{t("Open Punjabi Samvad on Facebook")} <ExternalLink size={16} /></a>
    </div>
    <div className="facebook-frame-wrap">
      <a className="facebook-page-bar" href={facebookPage} target="_blank" rel="noreferrer" aria-label="Visit Punjabi Samvad on Facebook">
        ...
        <span><strong>Punjabi Samvad</strong><small>{t("Official Facebook updates")}</small></span>
