...
export default function AboutPage() {
  ...
  return <>
    <section className="about-editorial-hero">
      ...
      <div className="container about-editorial-intro">
        <div className="about-hero-copy">
          <span className="eyebrow">{t("Who we are")}</span>
          <h1>{t("Dialogue that moves communities forward.")}</h1>
          <p>{t("Punjabi Samvad is a national non-governmental organisation rooted in Punjab. We address social issues through education, public awareness, community participation and creative communication.")}</p>
    ...
    <section className="section about-roots-section">
      <div className="container about-roots-layout">
        ...
        <div className="about-roots-copy">
          ...
          <p>{t("Punjabi Samvad began when Amit Bawa and Jyoti Bawa decided to respond to gender discrimination and other social concerns they saw around them. The organisation was formally registered in 2009.")}</p>
          <p>{t("What started as a local effort has grown into more than two decades of work with women, children, young people, schools, communities and institutions.")}</p>
          <blockquote><strong>{t("Samvad means dialogue.")}</strong> {t("For us, it is a practical way to help people discuss subjects that are often ignored, misunderstood or surrounded by stigma.")}</blockquote>
        </div>
      </div>
    </section>
    ...
    <section className="section about-method-section">
      <div className="container">
        <div className="about-method-head"><div><span className="eyebrow">{t("How we work")}</span><h2>{t("Start by listening. Build with care. Learn as we go.")}</h2></div><p>{t("Workshops, expert sessions, theatre, film, literature, internships and institutional partnerships are chosen for the people and the issue, not for a fixed template.")}</p></div>
        <div className="about-method-line">
          <div className="about-method-item"><Ear /><h3>{t("Listen")}</h3><p>{t("Understand the people, context and need before designing an activity.")}</p></div>
          <div className="about-method-item"><UsersRound /><h3>{t("Collaborate")}</h3><p>{t("Work with credible experts, community voices and institutional partners.")}</p></div>
          <div className="about-method-item"><Languages /><h3>{t("Make it useful")}</h3><p>{t("Use language and formats that make reliable information easier to act on.")}</p></div>
          <div className="about-method-item"><RefreshCw /><h3>{t("Improve")}</h3><p>{t("Track delivery, listen to response and refine programmes through experience.")}</p></div>
        </div>
      </div>
    </section>
    ...
    <section className="section about-direction-section">
      <div className="container">
        <div className="about-direction-grid">
          <article><span>{t("Our vision")}</span><h2>{t("An inclusive India where dignity and opportunity are not determined by gender or circumstance.")}</h2></article>
          <article><span>{t("Our mission")}</span><p>{t("We work through education, awareness, skills and creative communication while protecting the language, art and cultural heritage that help communities understand who they are.")}</p></article>
        </div>
        <div className="about-principles">
          <div><ShieldCheck /><h2>{t("The principles behind our work")}</h2></div>
          <ul>{principles.map(item => <li key={item}>{t(item)}</li>)}</ul>
        </div>
      </div>
    </section>
